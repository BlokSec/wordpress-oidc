...
class BlokSec_OIDC_Client {
	...
	function validate_authentication_request( $request ) {
		...
		if ( ! isset( $request['state'] ) ) {
			...
			return new WP_Error( 'missing-state', __( 'Missing state.', 'bloksec-oidc' ), $request );
		}

		if ( ! $this->check_state( $request['state'] ) ) {
			return new WP_Error( 'invalid-state', __( 'Invalid state.', 'bloksec-oidc' ), $request );
		}
	...
	function get_authentication_code( $request ) {
		if ( ! isset( $request['code'] ) ) {
			return new WP_Error( 'missing-authentication-code', __( 'Missing authentication code.', 'bloksec-oidc' ), $request );
		}
	...
	function request_authentication_token( $code ) {
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_authentication_token', __( 'Request for authentication token failed.', 'bloksec-oidc' ) );
		}
	...
	function request_new_tokens( $refresh_token ) {
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'refresh_token', __( 'Refresh token failed.', 'bloksec-oidc' ) );
		}
	...
	function get_token_response( $token_result ) {
		if ( ! isset( $token_result['body'] ) ) {
			return new WP_Error( 'missing-token-body', __( 'Missing token body.', 'bloksec-oidc' ), $token_result );
		}
		...
		if ( is_null( $token_response ) ) {
			return new WP_Error( 'invalid-token', __( 'Invalid token.', 'bloksec-oidc' ), $token_result );
		}
	...
	function request_userinfo( $access_token ) {
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_userinfo', __( 'Request for userinfo failed.', 'bloksec-oidc' ) );
		}
	...
	function get_id_token_claim( $token_response ) {
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new WP_Error( 'no-identity-token', __( 'No identity token.', 'bloksec-oidc' ), $token_response );
		}
		...
		if ( ! isset( $tmp[1] ) ) {
			return new WP_Error( 'missing-identity-token', __( 'Missing identity token.', 'bloksec-oidc' ), $token_response );
		}
	...
	function validate_id_token_claim( $id_token_claim ) {
		if ( ! is_array( $id_token_claim ) ) {
			return new WP_Error( 'bad-id-token-claim', __( 'Bad ID token claim.', 'bloksec-oidc' ), $id_token_claim );
		}

		// Validate the identification data and it's value.
		if ( ! isset( $id_token_claim['sub'] ) || empty( $id_token_claim['sub'] ) ) {
			return new WP_Error( 'no-subject-identity', __( 'No subject identity.', 'bloksec-oidc' ), $id_token_claim );
		}
	...
	function get_user_claim( $token_response ) {
		...
		if ( is_wp_error( $user_claim_result ) || ! isset( $user_claim_result['body'] ) ) {
			return new WP_Error( 'bad-claim', __( 'Bad user claim.', 'bloksec-oidc' ), $user_claim_result );
		}
	...
	function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ) {
			return new WP_Error( 'invalid-user-claim', __( 'Invalid user claim.', 'bloksec-oidc' ), $user_claim );
		}

		// Allow for errors from the IDP.
		if ( isset( $user_claim['error'] ) ) {
			$message = __( 'Error from the IDP.', 'bloksec-oidc' );
		...
		if ( $id_token_claim['sub'] !== $user_claim['sub'] ) {
			return new WP_Error( 'incorrect-user-claim', __( 'Incorrect user claim.', 'bloksec-oidc' ), func_get_args() );
		}
		...
		if ( ! $login_user ) {
			return new WP_Error( 'unauthorized', __( 'Unauthorized access.', 'bloksec-oidc' ), $login_user );
		}
