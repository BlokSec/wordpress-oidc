...
class OpenID_Connect_Generic_Client_Wrapper {
	...
	function ensure_tokens_still_fresh() {
		...
		if ( ! $refresh_token || ( $refresh_expires && $current_time > $refresh_expires ) ) {
			...
			if ( $this->settings->redirect_on_logout ) {
				$this->error_redirect( new WP_Error( 'access-token-expired', __( 'Session expired. Please login again.', 'bloksec-oidc' ) ) );
			}
	...
	function validate_user( $user ) {
		...
		if ( ! is_a( $user, 'WP_User' ) || ! $user->exists() ) {
			return new WP_Error( 'invalid-user', __( 'Invalid user.', 'bloksec-oidc' ), $user );
		}
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( ! empty( $this->settings->identity_key ) && isset( $user_claim[ $this->settings->identity_key ] ) ) {
			...
			return new WP_Error( 'no-username', __( 'No appropriate username found.', 'bloksec-oidc' ), $user_claim );
		}
		...
		if ( empty( $transliterated_username ) ) {
			return new WP_Error( 'username-transliteration-failed', sprintf( __( 'Username %1$s could not be transliterated.', 'bloksec-oidc' ), $desired_username ), $desired_username );
		}
		$normalized_username = strtolower( preg_replace( '/[^a-zA-Z0-9 _.\-@]/', '', $transliterated_username ) );
		if ( empty( $normalized_username ) ) {
			return new WP_Error( 'username-normalization-failed', sprintf( __( 'Username %1$s could not be normalized.', 'bloksec-oidc' ), $transliterated_username ), $transliterated_username );
		}
	...
	private function get_nickname_from_claim( $user_claim ) {
		...
		if ( empty( $desired_nickname ) ) {
			return new WP_Error( 'no-nickname', sprintf( __( 'No nickname found in user claim using key: %1$s.', 'bloksec-oidc' ), $this->settings->nickname_key ), $this->settings->nickname_key );
		}
				...
				if ( ! isset( $user_claim[ $key ] ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error(
							...
							__( 'User claim incomplete.', 'bloksec-oidc' ),
							array(
	...
	function create_new_user( $subject_identity, $user_claim ) {
		...
		if ( $values_missing && isset( $user_claim['access_token'] ) && ! empty( $this->settings->endpoint_userinfo ) ) {
			...
			if ( is_wp_error( $user_claim_result ) ) {
				return new WP_Error( 'bad-user-claim-result', __( 'Bad user claim result.', 'bloksec-oidc' ), $user_claim_result );
			}
		...
		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.', 'bloksec-oidc' ), $create_user );
		}
		...
		if ( is_wp_error( $uid ) ) {
			return new WP_Error( 'failed-user-creation', __( 'Failed user creation.', 'bloksec-oidc' ), $uid );
		}
